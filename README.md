# Any2Nmap: Компоненты для Браузерного Расширения

Этот проект — современная, полностью переписанная версия оригинального конвертера [any2nmap от Coder-ak](https://github.com/Coder-ak/any2nmap).
...
*   **Безопасная интеграция с Яндексом:** Авторизация реализована через современный и безопасный API для расширений — `chrome.identity`.
...
*   `manifest_instructions.md`: Подробные инструкции по модификации `manifest.json` вашего расширения для подключения этих компонентов.
